#!/usr/bin/env bash
#
# Build the Hugging Face Whisper inference image and push it to ECR.
#
# Steps:
#   1. Download the model snapshot (if not already present).
#   2. Log in to the AWS DLC ECR (to pull the base) and to your ECR.
#   3. Create the target ECR repo if needed.
#   4. Cross-build for linux/amd64 and push.
#
# Run from anywhere; the script cd's to the repo root (build context).
#
# Usage:
#   huggingface/build_and_push.sh [REGION] [REPO_NAME] [TAG]
# Defaults: REGION=ap-south-1, REPO_NAME=whisper-hf, TAG=latest

set -euo pipefail
cd "$(dirname "$0")/.."   # repo root = build context

REGION="${1:-ap-south-1}"
REPO_NAME="${2:-whisper-hf}"
TAG="${3:-latest}"

# AWS account that hosts the Hugging Face DLC base images (same in most regions).
DLC_ACCOUNT="763104351884"
BASE_IMAGE="${DLC_ACCOUNT}.dkr.ecr.${REGION}.amazonaws.com/huggingface-pytorch-inference:2.1.0-transformers4.37.0-gpu-py310-cu118-ubuntu20.04"

PY="${PY:-python3}"

ACCOUNT_ID="$(aws sts get-caller-identity --query Account --output text)"
TARGET_REGISTRY="${ACCOUNT_ID}.dkr.ecr.${REGION}.amazonaws.com"
TARGET_IMAGE="${TARGET_REGISTRY}/${REPO_NAME}:${TAG}"

echo "Region:        ${REGION}"
echo "Base image:    ${BASE_IMAGE}"
echo "Target image:  ${TARGET_IMAGE}"

if [ ! -f "model_artifacts/snapshot/config.json" ]; then
  echo "==> Downloading model snapshot..."
  "${PY}" common/download_model.py
else
  echo "==> Model snapshot already present, skipping download."
fi

echo "==> Logging in to DLC ECR (${DLC_ACCOUNT})..."
aws ecr get-login-password --region "${REGION}" \
  | docker login --username AWS --password-stdin "${DLC_ACCOUNT}.dkr.ecr.${REGION}.amazonaws.com"

echo "==> Logging in to target ECR (${ACCOUNT_ID})..."
aws ecr get-login-password --region "${REGION}" \
  | docker login --username AWS --password-stdin "${TARGET_REGISTRY}"

if ! aws ecr describe-repositories --region "${REGION}" --repository-names "${REPO_NAME}" >/dev/null 2>&1; then
  echo "==> Creating ECR repo ${REPO_NAME}..."
  aws ecr create-repository --region "${REGION}" --repository-name "${REPO_NAME}" \
    --image-scanning-configuration scanOnPush=true >/dev/null
fi

echo "==> Building and pushing (linux/amd64, no attestations)..."
docker buildx inspect whisper-builder >/dev/null 2>&1 || docker buildx create --name whisper-builder >/dev/null
docker buildx use whisper-builder

docker buildx build \
  --platform linux/amd64 \
  --provenance=false \
  --sbom=false \
  --build-arg "BASE_IMAGE=${BASE_IMAGE}" \
  --file huggingface/Dockerfile \
  --tag "${TARGET_IMAGE}" \
  --push \
  .

echo ""
echo "Pushed: ${TARGET_IMAGE}"
echo "Deploy: python common/deploy_from_ecr.py --image-uri ${TARGET_IMAGE} \\"
echo "          --region ${REGION} --role <role-arn> --endpoint-name whisper-hf"
